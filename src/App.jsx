    ...
    showTimerRef.current = setTimeout(() => {
      ...
      expireTimerRef.current = setTimeout(() => {
        ...
        setStats(prev => {
          ...
          return { ...prev, redResisted: prev.redResisted + 1 }
        })

        setMessage(color === 'blue' ? '놓쳤습니다.' : '좋습니다. 빨간색을 참았습니다.')

        // 중요: 파란색 TAP 기회만 라운드에 포함
        // 빨간색 STOP은 참아도 라운드 증가 없음
        if (color === 'blue') {
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <InfoCard
            title="게임 방법"
            body="파란색이 들어온 칸만 누르세요. 빨간색이 들어오면 누르지 마세요. 빨간색은 라운드에 포함되지 않고, 파란색 TAP 기회만 라운드에 포함됩니다."
          />
          <InfoCard
            title="점수 기준"
        ...
        <article className="rounded-3xl border border-slate-800 bg-slate-900 p-6 leading-7 text-slate-300">
          ...
          <p className="mt-3">
            파란색 TAP 기회만 라운드에 포함되며, 빨간색 STOP은 방해 신호로만 작동합니다.
            따라서 사용자는 정해진 라운드 수만큼 실제 반응 기회를 수행하면서 억제 능력도 함께 평가받을 수 있습니다.
          </p>
          <p className="mt-3">
